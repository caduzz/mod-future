...
import net.caduzz.futuremod.block.entity.CheckersBlockEntity;
import net.caduzz.futuremod.block.entity.CheckersBlockEntity.Piece;
...
public final class CheckersBlockRenderer implements BlockEntityRenderer<CheckersBlockEntity> {
    ...
    public void render(CheckersBlockEntity be, float partialTick, PoseStack poseStack, MultiBufferSource buffer,
        ...
        for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
                ...
                int[] rgb = pieceRgb(p);
                drawBox(poseStack.last(), consumer, cx, cy, cz, HALF_W * scale, HALF_H * scale, rgb[0], rgb[1], rgb[2],
                        255, packedLight);
            }
        }
        ...
        if (be.hasActiveAnimation()) {
            ...
            int[] rgb = pieceRgb(ap);
            drawBox(poseStack.last(), consumer, cx, cy, cz, HALF_W, HALF_H, rgb[0], rgb[1], rgb[2], 255, packedLight);
        }
    ...
    private static int[] pieceRgb(Piece p) {
        return switch (p) {
            case WHITE_MAN, WHITE_KING -> new int[] { 238, 238, 245 };
            case BLACK_MAN, BLACK_KING -> new int[] { 38, 38, 48 };
